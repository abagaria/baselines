...
class MlpPolicy(object):
    ...
    def get_option(self, ob):
        try:
            ...
            if np.isnan(op_prob).any():
                chosen_option = np.random.choice(range(len(op_prob)))
            else:
                chosen_option = np.random.choice(range(len(op_prob)), p=op_prob)
        except:
            pdb.set_trace()
    ...
    def get_term_adv(self, ob, curr_opt):
        ...
        try:
            ...
            if np.isnan(op_prob).any():
                ad_term = (vals[curr_opt[0]] - np.sum((vals),axis=0) + self.dc),  ( vals[curr_opt[0]] - np.sum((vals),axis=0) )
            else:
                ad_term = (vals[curr_opt[0]] - np.sum((op_prob*vals),axis=0) + self.dc),  ( vals[curr_opt[0]] - np.sum((op_prob*vals),axis=0) )
        except:
            pdb.set_trace()
        return ad_term
