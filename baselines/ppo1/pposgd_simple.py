...
def traj_segment_generator(pi, env, horizon, stochastic, num_options,saves,results,rewbuffer,dc, training_mode):
    ac = env.action_space.sample() # not used, just so we have the datatype
    new = True # marks if we're on first timestep of an episode
    ob = env.reset(training_time=training_mode)
    print("Sampled initial state: ", ob[-4:-2])
    ...
    for t in range(horizon):
        ...
        # Sometimes pi returns NaN, dont want to crash so treating it as a noop
        if np.isnan(ac).any():
            ac = np.array([0.] * 8)
        ...
        if new:
            ...
            rew = -1. * env.distance_to_goal_position(new_position)

        episodic_score += rew
        episodic_duration += 1

        # Not diving by 10 because I have verified that low reward scale
        # causes problems with ant maze
    ...
    return {"ob" : obs, "rew" : rews, "realrew": realrews, "vpred" : vpreds, "new" : news,
            "ac" : acs, "opts" : opts, "prevac" : prevacs, "nextvpred": vpred * (1 - new),
            "ep_rets" : [cur_ep_ret], "ep_lens" : [cur_ep_len], 'term_p': term_p, 'value_val': value_val,
             "opt_dur": opt_duration, "optpol_p":optpol_p, "logstds": logstds, "episodic_score": episodic_score,
             "episodic_duration": episodic_duration}
...
def learn(env, policy_func, *,
    ...
    if wsaves:
        ...
        for i in range(len(files)):
            src = os.path.expanduser('~/git-repos/baselines/baselines/ppo1/') + files[i]
            dest = os.path.expanduser('~/git-repos/baselines/baselines/ppo1/') + dirname
    ...
    for episodes_so_far in range(max_episodes):
        ...
        seg = seg_gen = traj_segment_generator(pi, env, timesteps_per_batch, stochastic=True, num_options=num_options,saves=saves,results=results,rewbuffer=rewbuffer,dc=dc, training_mode=True)
        
        # Log reward and duations
        episodic_score = seg["episodic_score"]
        episodic_rewards.append(episodic_score)
        episodic_duration = seg["episodic_duration"]
        ...
        if episodes_so_far % 10 == 0:
            print("="*80)
            seg = seg_gen = traj_segment_generator(pi, env, timesteps_per_batch, stochastic=True, num_options=num_options,saves=saves,results=results,rewbuffer=rewbuffer,dc=dc, training_mode=False)

            # Log reward and duations
            val_score = seg["episodic_score"]
            validation_rewards.append(val_score)
            val_duration = seg["episodic_duration"]
            validation_durations.append(val_duration)
            
            print("Validation Score: {:.2f}\tValidation Duration: {}".format(val_score, val_duration))
            print("="*80)
    ...
    return episodic_rewards, episodic_durations, validation_rewards, validation_durations
